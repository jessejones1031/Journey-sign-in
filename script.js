...
document
  .getElementById("add-teen-button")
  .addEventListener("click", showAddTeenForm);
document
  .getElementById("cancel-add-teen")
  .addEventListener("click", hideAddTeenForm);
document
  .getElementById("attendance-form")
  .addEventListener("submit", addNewTeen);
document
...
async function searchTeen() {
  ...
  const q1 = query(
    collection(db, "teens"),
    where("firstNameLower", "==", searchName)
  );
  const q2 = query(
    collection(db, "teens"),
    where("lastNameLower", "==", searchName)
  );
